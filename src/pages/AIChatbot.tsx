...
const AIChatbot = () => {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          else if (model.includes("openrouter")) setApiMode("openrouter");
      ...
      if (quotaExceeded) {
        ...
        const quotaMessage = {
          ...
          role: "assistant" as const,
          content: "⚠️ **Genesis AI — Live Service Unavailable**\n\nThe hosted model routes are unavailable right now, so I'm running in **Enhanced Local Mode**. I still know Aman's projects, metrics, and career details from the built-in knowledge base.",
          timestamp: new Date(),
          isQuotaError: true,
        };
  ...
  const getStatusBadge = () => {
    if (apiMode === "gemini") {
      ...
      return { text: "Local Mode", bg: "bg-orange-500/10 text-orange-400 border-orange-500/20", dot: "bg-orange-400 animate-bounce" };
    } else {
          ...
          <div className="absolute top-[73px] left-0 right-0 z-20 bg-slate-950/95 border-b border-white/10 p-4 backdrop-blur-xl animate-in slide-in-from-top duration-300">
            <form onSubmit={handleSaveApiSettings} className="max-w-2xl mx-auto space-y-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                Choose the OpenRouter fallback model visitors should use when the primary hosted route is unavailable.
              </p>
              {apiMode === "quota_exceeded" && (
                <div className="flex items-center gap-2 text-[11px] text-orange-400 bg-orange-500/10 border border-orange-500/20 rounded-lg p-2">
                  ...
                  <span>The hosted routes are currently unavailable. The local knowledge base will answer until service resumes.</span>
                </div>
              )}
            ...
            <Input
              ...
              placeholder={
                apiMode === "gemini" 
                  ...
                  : apiMode === "quota_exceeded"
                  ? "Live service unavailable - using the local knowledge base..."
